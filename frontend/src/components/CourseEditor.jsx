...
const RichTextEditor = ({ value, onChange, placeholder }) => {
  ...
  const insertImage = () => {
    ...
    if (!url) return;
    const width = prompt('Enter image width (e.g. 400, 100%, or leave blank for auto):', '100%');
    const height = prompt('Enter image height (e.g. 300, auto, or leave blank for auto):', 'auto');
    const w = width || '100%';
    const h = height || 'auto';
    const sizeStyle = `width:${w.includes('%') || w.includes('px') ? w : w + 'px'};height:${h.includes('%') || h.includes('px') || h === 'auto' ? h : h + 'px'};`;
    execCmd('insertHTML', `<img src="${url}" alt="image" style="max-width:100%;border-radius:8px;margin:8px 0;display:block;${sizeStyle}" />`);
  };
  ...
  return (
    <div className="border border-outline-variant rounded-xl overflow-hidden bg-white">
      ...
      <div className="flex flex-wrap items-center gap-0.5 p-2 border-b border-outline-variant/50 bg-surface-container-lowest">
        ...
        <ToolBtn icon={ImageIcon} title="Insert Image" onClick={insertImage} />
